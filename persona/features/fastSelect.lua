...
local function drawCenterElements(zoom)
    ...
    -- Background images
    persona_localanimation.displayImage(centerOffset,
        "/celestial/system/terrestrial/biomes/midnight/maskie2.png", 0.5 / zoom, {255, 255, 255, CONSTANTS.COLORS.CENTER_ALPHA})
    persona_localanimation.displayImage(centerOffset,
...
local function drawHeaderText(zoom)
    ...
    persona_localanimation.displayText(headerPosition,
        "^shadow;FastSelect " .. headerText .. "^reset;", CONSTANTS.TEXT_SCALES.HEADER / zoom)
end
...
local function drawOptionBorders(zoom)
    for i = 0, #state.options - 1 do
        ...
        persona_localanimation.displayLine(lineStart, lineEnd, CONSTANTS.COLORS.BORDER, CONSTANTS.LINE_WIDTHS.NORMAL / zoom)
    end
end
...
local function drawSelectedOption(zoom)
    if state.selectedOption <= 0 then return end
    ...
    persona_localanimation.displayLine(innerStart, outerStart, CONSTANTS.COLORS.SELECTED, CONSTANTS.LINE_WIDTHS.SELECTED / zoom)
    persona_localanimation.displayLine(innerEnd, outerEnd, CONSTANTS.COLORS.SELECTED, CONSTANTS.LINE_WIDTHS.SELECTED / zoom)
    
    -- Draw selected text
    local selectedTextAngle = getTextAngleForOption(state.selectedOption, #state.options)
    local selectedTextPosition = getPositionAtDistance(state.selectLocation, selectedTextAngle, CONSTANTS.TEXT_RADIUS / zoom)
    
    persona_localanimation.displayText(selectedTextPosition,
        "^" .. CONSTANTS.COLORS.SELECTED .. ";" .. state.options[state.selectedOption] .. "^reset;", 
        CONSTANTS.TEXT_SCALES.SELECTED / zoom)
end
...
local function drawCenterText(distanceToCursor, zoom)
    ...
    local textScale = distanceToCursor <= CONSTANTS.INNER_RADIUS / zoom and CONSTANTS.TEXT_SCALES.SELECTED or CONSTANTS.TEXT_SCALES.NORMAL
