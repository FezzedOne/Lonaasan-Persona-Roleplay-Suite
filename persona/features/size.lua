...
function persona_feature_size.toCursor(zoom, shift)
    ...
    persona_localanimation.displayText(
        ...
        string.format("^shadow;Size: %.2f^reset;", playerSize),
        1.5 / zoom
    )
end
...
function persona_feature_size.update()
    ...
    if playerSize == 0 then playerSize = status.statusProperty("personaSize", 1) end
    if playerSize ~= 1 then
        if not effects["personaSize"] then status.addEphemeralEffect("personaSize", math.huge) end
        mcontroller.controlParameters({
            standingPoly = persona_math.scalePoly({
                { -0.75, -2.0 },
                { -0.35, -2.5 },
                { 0.35, -2.5 },
                { 0.75, -2.0 },
                ...
                { -0.35, 1.22 },
                { -0.75, 0.65 },
            }, tonumber(playerSize) or 1.0),
            crouchingPoly = persona_math.scalePoly({
                { -0.75, -2.0 },
                { -0.35, -2.5 },
                { 0.35, -2.5 },
                { 0.75, -2.0 },
...
function persona_feature_size.reset()
    local effects = status.activeUniqueStatusEffectSummary()
    if effects["personaSize"] then status.removeEphemeralEffect("personaSize") end
