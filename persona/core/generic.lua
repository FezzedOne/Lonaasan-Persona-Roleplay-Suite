...
function init(...)
    ...
    if
        not status.statusProperty("personaIgnored", false)
        and player.equippedTech("body") ~= "personabody"
        and player.equippedTech("legs") ~= "personalegs"
        and player.equippedTech("head") ~= "personahead"
    then
        player.interact("ScriptPane", "/interface/persona/techEquip/techEquip.config")
    end

    if client == "XStarbound" or client == "OpenStarbound" then
        -- Allows xStarbound and OpenStarbound users to use the `"SwimIdle"` state.
        table.insert(stateOptions, {
            name = "SwimIdle",
            description = "Swim Idle",
        })
    end
...
function update(dt, ...)
    ...
    if input.bindDown("persona", "stickToEntity") then
        ...
        interface.queueMessage(
            "Stick to entity: " .. (stickToEntityActive and "^green;Enabled^reset;" or "^red;Disabled^reset;")
        )

        if not stickToEntityActive then persona_feature_position.reset() end
    end
    ...
    if input.bindDown("persona", "flight") then
        ...
        if not flightActive then mcontroller.controlParameters({
            gravityEnabled = true,
        }) end
    end

    if input.bindDown("persona", "teleport") then mcontroller.setPosition(player.aimPosition()) end

    if input.bindDown("persona", "playerRadar") then playerRadarActive = not playerRadarActive end
    if input.bindDown("persona", "stickymotes") then
        persona_feature_stickymotes.reset()
        stickymotesActive = not stickymotesActive

        interface.queueMessage(
            "Stickymotes: " .. (stickymotesActive and "^green;Enabled^reset;" or "^red;Disabled^reset;")
        )
    end
    ...
    if fastSelectActive and shift and not lastShiftState then
        ...
        if currentTableIndex > #optionTables then currentTableIndex = 1 end
        -- Update the wheel with the new options immediately
        wheelOptions = optionTables[currentTableIndex]
        persona_feature_fastSelect.show(wheelOptions, zoom)
    end

    if os.__tech and lastParentState then os.__tech.setParentState(lastParentState) end -- Ensures Persona's state override sticks when it's set.

    if input.bindDown("persona", "fastSelectAdd") then table.insert(wheelOptions, "test_" .. #wheelOptions + 1) end
    ...
    if not fastSelectActive and lastFastSelectState then
        ...
        if result then
            ...
            elseif contains(stateOptions, result) then
                ...
                if os.__tech then
                    if not state or lastParentState == state then state = nil end
    ...
    if playerRadarActive then
        if os.__localAnimator then
            ...
            persona_localanimation.displayText(
                vec2.add(mcontroller.position(), { 0, 28 / zoom }),
                "^shadow;PlayerRadar^reset;" or "",
                1.5 / zoom
            )
    ...
    if flightActive then persona_feature_position.flight(shiftDown, alt) end

    if stickToEntityActive then persona_feature_position.stickToEntity() end

    if stickymotesActive then persona_feature_stickymotes.update() end
    ...
    -- Tells FezzedTech to «let go» of its control over the parent state and player rotation whenever Persona is controlling either of those.
    status.setStatusProperty("ignoreFezzedTech", isRotated or not not lastParentState)
